    ...
    def parse_class(self, class_root: ET.Element, filepath: str) -> None:
        ...
        if structs is not None:
            for struct in structs:
                ...
                struct_desc = struct.find("description")
                if struct_desc is not None and struct_desc.text:
                    struct_def.description = struct_desc.text
                ...
                for member in struct.findall("member"):
                    ...
                    member_description = member.text or None

                    member_def = PropertyDef(
                        member_name, member_type, None, None, member_description, member_default, None
                    )
                    struct_def.members[member_name] = member_def
    ...
    with open(
        ...
        if len(class_def.structs) > 0:
            ...
            for struct_def in class_def.structs.values():
                ...
                # if struct_def.description is not None and struct_def.description.strip() != "":
                #     f.write(f"{format_text_block(struct_def.description.strip(), struct_def, state)}")
                # else:
                #     f.write(".. container:: contribute\n\n\t")
                #     f.write(
                #         translate(
                ...
                for member_name, member in struct_def.members.items():
                    ...
                    # if member.description is not None and member.description.strip() != "":
                    #     f.write(f"{format_text_block(member.description.strip(), member, state)}")
                    # else:
                    #     f.write(".. container:: contribute\n\n\t")
                    #     f.write(
                    #         translate(
                    #             "There is currently no description for this struct member. Please help us by :ref:`contributing one <doc_updating_the_class_reference>`!"
                    #         )
                    #         + "\n\n"
                    #     )
